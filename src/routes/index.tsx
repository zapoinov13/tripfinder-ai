...
import { Building2, Flame, ShieldCheck, Sparkles, Wallet } from "lucide-react";
...
function Index() {
  ...
  return (
    <SiteLayout>
      {/* Mobile first screen ends on B2B; destinations start on screen 2 */}
      <div className="flex min-h-[calc(100svh-4rem-5.75rem-env(safe-area-inset-bottom))] flex-col md:min-h-0">
        <section className="md:container-page md:py-14">
          <div className="relative overflow-hidden md:rounded-[2.5rem]">
            <img
              ...
              className="h-[min(36svh,13.5rem)] w-full object-cover object-[center_35%] animate-soft-zoom md:h-[32rem]"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-ink via-ink/50 to-ink/15" />

            <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 px-4 pb-4 pt-10 text-primary-foreground md:gap-5 md:p-10">
              <p className="hidden animate-fade-up font-display text-3xl font-semibold tracking-tight md:block">
                TourGo
              </p>

              <h1 className="animate-fade-up font-display text-[1.65rem] font-semibold leading-[1.08] tracking-tight [animation-delay:80ms] sm:text-5xl md:text-6xl">
                Сравните цены
                <br />и купите выгоднее
              </h1>

              <p className="animate-fade-up max-w-md text-[14px] leading-snug text-primary-foreground/80 [animation-delay:140ms] md:text-xl md:leading-relaxed">
              ...
              <Link
                to="/ai-search"
                search={{} as never}
                className="animate-fade-up mt-0.5 flex h-11 w-full items-center gap-3 rounded-2xl bg-background px-3.5 text-left text-foreground shadow-lg active:scale-[0.99] [animation-delay:200ms] md:hidden"
              >
        ...
        <section className="container-page flex flex-1 flex-col pt-3 md:pt-0">
          ...
          <div className="grid auto-rows-fr grid-cols-2 items-stretch gap-2 md:mt-10 md:grid-cols-3 md:gap-4">
            {travelScenarios.map((item) => (
              <Link
                ...
                to={item.to}
                search={{} as never}
                className="surface-card flex h-full min-h-[5.75rem] flex-col items-start justify-between gap-1.5 p-3 text-left md:min-h-[11.5rem] md:p-6 md:transition-transform md:hover:-translate-y-1"
              >
                <span className="grid size-9 place-items-center rounded-2xl bg-primary-soft text-primary md:size-14">
                  <item.icon className="size-[18px] md:size-7" />
                </span>
                <span className="min-w-0 w-full">
                  <span className="block font-display text-[15px] font-semibold leading-tight md:hidden">
                    {item.shortTitle ?? item.title}
                  </span>
                  <span className="hidden font-display text-2xl font-semibold leading-snug md:block">
                    {item.title}
                  </span>
                  <span className="mt-0.5 block text-[11px] leading-snug text-foreground/60 md:mt-1 md:text-base md:leading-relaxed">
          ...
          <Link
            to={b2bNav.to}
            className="mt-auto flex items-center gap-3 rounded-2xl bg-ink px-4 py-3.5 text-primary-foreground md:mt-8 md:rounded-[1.75rem] md:gap-4 md:px-8 md:py-6"
          >
            <span className="min-w-0">
              ...
              <span className="mt-1 block font-display text-[15px] font-semibold leading-snug md:mt-2 md:text-2xl">

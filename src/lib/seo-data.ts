 ...
 * сети превратит живую компанию в удалённую. Отказ базы разбираем по её же
 * коду ошибки: негодный идентификатор — это «нет строки», а не «нет связи».
 */
async function selectOne<T>(
  ...
  try {
    ...
    if (!response.ok) {
      // Не всякий отказ значит «база недоступна», и разница тут принципиальна.
      //
      // Если в адресе не идентификатор, а мусор («/company/takoy-net»), Postgres
      // отвечает 22P02: такое значение не приводится к типу колонки, то есть
      // строки с ним не существует и существовать не может. Это честный 404.
      //
      // А вот 42703 («нет такой колонки») — наша собственная ошибка в запросе.
      // Отдавать по ней 404 нельзя: одна опечатка в списке колонок увела бы в
      // «не найдено» все живые компании разом. Такое пишем в лог и показываем
      // страницу — каталог подтянется в браузере.
      ...
      console.warn(
        `[seo-data] ${table}: ${response.status} ${response.statusText}` +
          `${code ? ` (${code})` : ""} — запрос к представлению не прошёл`,
      );
      return undefined;
    }

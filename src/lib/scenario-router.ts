...
function detectScenario(query: string): TravelScenarioId {
  ...
  // «Что посмотреть», «куда сходить» — это про впечатления на месте, а не про
  // путёвку. Проверяем последними и только целыми оборотами: если человек уже
  // назвал отель, машину или зал, его слово важнее, а «посмотреть туры»
  // остаётся турами.
  if (has("что посмотрет", "куда сходит", "достопримечательност")) {
    return "excursions";
  }
  return "tours";
}

...
--
-- Документы компаний: хранилище, права доступа и очередь проверки для
-- админа. Вставьте целиком и нажмите Run — внизу будет таблица «часть / итог».
-- ===========================================================================
...
-- ---------------------------------------------------------------------------
select 'Бакет company-docs (приватный)' as "часть",
       case when exists (
         select 1 from storage.buckets where id = 'company-docs' and public = false
       ) then 'есть' else 'НЕТ' end as "итог"
union all
select 'Таблица company_documents',
       case when to_regclass('public.company_documents') is not null then 'есть' else 'НЕТ' end
union all
select 'RLS на company_documents включена',
       case when coalesce((
         select relrowsecurity from pg_class where oid = to_regclass('public.company_documents')
       ), false) then 'есть' else 'НЕТ' end
union all
select 'Правила доступа к company_documents (4 шт.)',
       case when (
         select count(*) from pg_policies
         where schemaname = 'public' and tablename = 'company_documents'
       ) >= 4 then 'есть' else 'НЕТ' end
union all
select 'Правила доступа к файлам в бакете (4 шт.)',
       case when (
         select count(*) from pg_policies
         where schemaname = 'storage' and tablename = 'objects'
           and policyname like 'company_docs_%'
       ) >= 4 then 'есть' else 'НЕТ' end
union all
select 'Защита: партнёр не проверяет сам себя',
       case when exists (
         select 1 from pg_trigger
         where tgrelid = to_regclass('public.company_documents')
           and tgname = 'company_documents_guard_review'
       ) then 'есть' else 'НЕТ' end
union all
select 'Очередь проверки admin_company_documents',
       case when to_regprocedure('public.admin_company_documents()') is not null
            then 'есть' else 'НЕТ' end
union all
select 'Решение review_company_document',
       case when to_regprocedure('public.review_company_document(uuid, text, text)') is not null
            then 'есть' else 'НЕТ' end;

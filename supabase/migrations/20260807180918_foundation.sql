...
-- Helpers (security definer in private-ish public with tight grants)
...
as $$
  select exists (
    ...
    where id = auth.uid()
      and role in ('PLATFORM_ADMIN','PLATFORM_MANAGER')
      and status = 'active'
  );
$$;
...
create policy config_admin_write on public.platform_config for update using (public.is_platform_admin());
...
create policy members_read on public.organization_members for select using (
  public.is_platform_admin() or organization_id = public.my_org_id()
);
create policy members_write on public.organization_members for all using (
  public.is_platform_admin() or organization_id = public.my_org_id()
);

-- Tourist private data
create policy favorites_own on public.favorites for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy comparisons_own on public.comparisons for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy alerts_own on public.price_alerts for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy ai_own on public.ai_searches for all using (user_id = auth.uid()) with check (user_id = auth.uid());
create policy notif_own on public.notifications for all using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy bookings_own_read on public.bookings for select using (
...
-- Operator tours write
create policy tours_operator_write on public.tour_offers for update using (
  public.is_platform_admin() or operator_org_id = public.my_org_id()
);
create policy tours_admin_all on public.tour_offers for insert with check (
  public.is_platform_admin() or operator_org_id = public.my_org_id()
);

create policy api_conn_org on public.operator_api_connections for all using (
  public.is_platform_admin() or organization_id = public.my_org_id()
) with check (public.is_platform_admin() or organization_id = public.my_org_id());

create policy sync_logs_org on public.sync_logs for select using (
  public.is_platform_admin() or organization_id = public.my_org_id()
);
create policy sync_logs_insert on public.sync_logs for insert with check (
  public.is_platform_admin() or organization_id = public.my_org_id()
);

create policy promotions_org on public.promotions for all using (
  public.is_platform_admin() or organization_id = public.my_org_id()
) with check (public.is_platform_admin() or organization_id = public.my_org_id());
...
create policy analytics_admin on public.analytics_events for select using (public.is_platform_admin());

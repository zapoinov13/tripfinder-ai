...
-- Column-scoped anonymous access to safe fields only
...
CREATE POLICY orgs_read_public_safe ON public.organizations
  FOR SELECT TO anon
  USING (status = 'APPROVED');

CREATE POLICY config_read_public_safe ON public.platform_config
  FOR SELECT TO anon
  USING (true);

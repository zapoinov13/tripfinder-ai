-- ===========================================================================
-- TourGo · свой счётчик посещаемости
--
-- Вставьте это целиком в SQL Editor и нажмите Run:
--   https://supabase.com/dashboard/project/mgyufoyornzbwvgdfojb/sql/new
--
-- Что это даёт: в админке → «Аналитика» появится раздел «Трафик» — сколько
-- людей пришло, откуда (поисковики, соцсети, реклама, прямые заходы), на
-- какие страницы и с каких устройств.
--
-- Сам сбор визитов работает и без этого запроса: страницы уже пишутся в базу.
-- Запрос нужен, чтобы их посчитать и показать.
--
-- Запускать можно повторно — ничего не сломается и не задвоится.
-- ===========================================================================
...
-- Проверка: должна вернуть одну строку с именем функции.
select p.proname as "Функция", 'готово' as "Статус"
from pg_proc p join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public' and p.proname = 'traffic_stats';

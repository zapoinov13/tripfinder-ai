-- ===========================================================================
-- TourGo · применить в Supabase SQL Editor одним куском
-- https://supabase.com/dashboard/project/mgyufoyornzbwvgdfojb/sql/new
--
-- Здесь два блока, оба ещё не применены:
--   1. Уведомления второй стороне создаёт база (триггеры).
--   2. Защита переписки и статуса записи от подделки.
--
-- Порядок важен: сначала уведомления, потом защита. Запускать можно целиком,
-- повторный запуск безопасен — всё пересоздаётся через create or replace и
